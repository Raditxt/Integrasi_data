  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: "Respons non-JSON atau kosong" }));
      const errorMessage =
        errorData.message ||
        `HTTP error! Status: ${response.status} - ${response.statusText}`;
      console.error(
        `Gagal melakukan permintaan ke ${url}:`,
        errorMessage,
        errorData
      );
    ...
    if (!error.message.includes("Operasi gagal")) {
      // Hindari duplikasi toast jika sudah ditangani di response.ok
...
async function loadStatistics() {
  ...
  try {
    ...
    if (stats) {
      ...
      statsContainer.innerHTML = `
          ...
          stats.avg_rating || 0
                ...
                <div class="bg-orange-100 p-6 rounded-lg shadow-md text-center">
                    ...
                    <p class="text-4xl font-bold text-orange-900">${stats.total_suppliers || 0
        }</p>
                </div>
                <div class="bg-red-100 p-6 rounded-lg shadow-md text-center">
                    <p class="text-lg text-red-700">Total Karyawan</p>
                    <p class="text-4xl font-bold text-red-900">${stats.total_employees || 0
        }</p>
                </div>
            `;
      ...
      renderDoughnutChart(
        "customerOrderDoughnutChart",
        customerOrderLabels,
        customerOrderData,
        "Distribusi Pelanggan vs Pesanan"
      );
    } else {
...
document.getElementById("form-produk").addEventListener("submit", async function (e) {
  ...
  const payload = {
    ...
    id_supplier: parseInt(document.getElementById("id_supplier_produk").value),
  };
  ...
  if (
    ...
    !payload.brand ||
    isNaN(payload.harga) ||
    isNaN(payload.id_sub_kategori) ||
    isNaN(payload.id_supplier)
  ) {
  ...
  try {
    ...
    const duplicateCheck = await apiFetch(
      `/produk?nama=${encodeURIComponent(payload.nama_produk)}&brand=${encodeURIComponent(payload.brand)}`
    );
    if (duplicateCheck && duplicateCheck.length > 0) {
      showToast("Produk dengan nama dan brand ini sudah ada!", "warning");
      return;
    }
...
async function loadProduk() {
  ...
  try {
    ...
    data.forEach((p) => {
      ...
      card.className =
        "bg-white border border-gray-200 rounded-lg shadow-md p-4 flex flex-col justify-between transform transition duration-300 hover:scale-[1.02] hover:shadow-lg";
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">${p.nama_produk || "N/A"
        }</h4>
            <p class="text-sm text-gray-600">Brand: <span class="font-medium">${p.brand || "N/A"
        }</span></p>
            <p class="text-blue-600 font-bold text-xl my-2">Rp${p.harga !== undefined && p.harga !== null
          ? parseFloat(p.harga).toLocaleString("id-ID")
          : "0"
        }</p>
            <p class="text-xs text-gray-500">ID Produk: <span class="font-mono">${p.id_produk || "N/A"
        }</span></p>
            <p class="text-xs text-gray-500">Sub Kategori ID: <span class="font-mono">${p.id_sub_kategori || "N/A"
        }</span></p>
            <p class="text-xs text-gray-500">Supplier ID: <span class="font-mono">${p.id_supplier || "N/A"
        }</span></p>
          </div>
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      ...
      // Deteksi error 409 (duplikat)
      if (error.response && error.response.status === 409) {
        showToast("Pelanggan dengan email atau nomor telepon tersebut sudah ada!", "error");
      } else {
...
async function loadPelanggan() {
  ...
  try {
    ...
    data.forEach((p) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">${p.nama_pelanggan || "N/A"
        }</h4>
            <p class="text-sm text-gray-600">ID Pelanggan: <span class="font-mono">${p.id_pelanggan || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Email: <span class="font-medium">${p.email || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Telepon: <span class="font-medium">${p.nomor_telepon || "N/A"
        }</span></p>
            <p class="text-xs text-gray-500 mt-2">Alamat: ${p.alamat || "N/A"
        }</p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
          </div>
...
document
  ...
  .addEventListener("submit", async function (e) {
    e.preventDefault();
    const payload = {
      id_pelanggan: parseInt(document.getElementById("id_pelanggan_pesanan").value),
    ...
    if (
      isNaN(payload.id_pelanggan) ||
      !payload.tanggal_pesanan ||
      isNaN(payload.total_pesanan)
    ) {
    ...
    try {
      // 🔍 Cek duplikasi
      const existing = await apiFetch("/pesanan");
      const isDuplicate = existing.some(p =>
        p.id_pelanggan === payload.id_pelanggan &&
        p.tanggal_pesanan === payload.tanggal_pesanan
      );

      if (isDuplicate) {
        showToast("Pesanan pelanggan ini pada tanggal tersebut sudah ada!", "warning");
        return;
      }
...
async function loadPesanan() {
  ...
  try {
    ...
    data.forEach((p) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">Pesanan ID: <span class="font-mono text-teal-700">${p.id_pesanan || "N/A"
        }</span></h4>
            <p class="text-sm text-gray-600">Pelanggan ID: <span class="font-mono">${p.id_pelanggan || "N/A"
        }</span></p>
            <p class="text-green-600 font-bold text-xl my-2">Rp${p.total_pesanan !== undefined && p.total_pesanan !== null
          ? parseFloat(p.total_pesanan).toLocaleString("id-ID")
          : "0"
        }</p>
            <p class="text-xs text-gray-500">Tanggal: Tanggal: ${p.tanggal_pesanan
...
// ============================== PENGIRIMAN ==============================
document
  .getElementById("form-pengiriman")
  .addEventListener("submit", async function (e) {
    e.preventDefault();
    const payload = {
      id_pesanan: parseInt(
        document.getElementById("id_pesanan_pengiriman").value
      ),
      ...
      id_karyawan: document.getElementById("id_karyawan_pengiriman").value
        ? parseInt(document.getElementById("id_karyawan_pengiriman").value)
        : null,
    };

    if (
      isNaN(payload.id_pesanan) ||
      !payload.kurir ||
      !payload.no_resi ||
      !payload.status_pengiriman ||
      !payload.tanggal_kirim
    ) {
      showToast(
        "Mohon lengkapi semua field pengiriman yang wajib diisi!",
        "warning"
      );
      return;
    }
    ...
    try {
      // 🔍 Cek apakah ID pesanan sudah punya pengiriman
      const existingByPesanan = await apiFetch(`/pengiriman/${payload.id_pesanan}`);
      if (existingByPesanan && existingByPesanan.id_pengiriman) {
        showToast("Pesanan ini sudah memiliki data pengiriman!", "warning");
        return;
      }
    
      // 🔍 Cek apakah no_resi sudah digunakan
      const allShipments = await apiFetch("/pengiriman");
      const duplicateResi = allShipments.find(p => p.no_resi === payload.no_resi);
      if (duplicateResi) {
        showToast("Nomor resi ini sudah digunakan!", "warning");
        return;
      }
    
      // ✅ Submit jika lolos duplikat
...
async function loadPengiriman() {
  ...
  try {
    ...
    data.forEach((p) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">Pengiriman ID: <span class="font-mono text-indigo-700">${p.id_pengiriman || "N/A"
        }</span></h4>
            <p class="text-sm text-gray-600">Pesanan ID: <span class="font-mono">${p.id_pesanan || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Kurir: <span class="font-medium">${p.kurir || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">No Resi: <span class="font-mono">${p.no_resi || "N/A"
        }</span></p>
            <p class="text-sm font-bold ${statusColorClass}">Status: ${p.status_pengiriman || "N/A"
        }</p>
            <p class="text-xs text-gray-500">Tanggal Kirim: Tanggal: ${p.tanggal_kirim
          ? new Date(p.tanggal_kirim).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })
          : 'N/A'
        }</p>
            <p class="text-xs text-gray-500">ID Karyawan: ${p.id_karyawan || "Tidak ada"
        }</p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
          </div>
...
// ============================== RATING ==============================
document
  .getElementById("form-rating")
  .addEventListener("submit", async function (e) {
    e.preventDefault();
    const payload = {
      id_pesanan: parseInt(document.getElementById("id_pesanan_rating").value), // ID unik
      id_pelanggan: parseInt(
        document.getElementById("id_pelanggan_rating").value
      ), // ID unik
    ...
    if (
      isNaN(payload.id_pesanan) ||
      isNaN(payload.id_pelanggan) ||
      isNaN(payload.rating_belanja) ||
      payload.rating_belanja < 1 ||
      payload.rating_belanja > 5
    ) {
      showToast(
        "Mohon lengkapi semua field rating dengan benar (Rating 1-5)!",
        "warning"
      );
      return;
    }
    ...
    try {
      // 🔍 Cek apakah rating untuk pesanan & pelanggan sudah pernah ada
try {
  const existingRatings = await apiFetch("/rating");
  const duplicate = existingRatings.find(
    (r) =>
      r.id_pesanan === payload.id_pesanan &&
      r.id_pelanggan === payload.id_pelanggan
  );
  if (duplicate) {
    showToast("Rating sudah pernah diberikan untuk pesanan ini!", "warning");
    return;
  }
} catch (error) {
  showToast("Gagal memeriksa duplikasi rating", "error");
  return;
}
...
async function loadRating() {
  ...
  try {
    ...
    data.forEach((r) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">Pesanan ID: <span class="font-mono text-purple-700">${r.id_pesanan || "N/A"
        }</span></h4>
            <p class="text-sm text-gray-600">Pelanggan ID: <span class="font-mono">${r.id_pelanggan || "N/A"
        }</span></p>
            <p class="text-yellow-500 text-2xl font-bold my-2">${getStarRating(
          r.rating_belanja || 0
        )} <span class="text-gray-700 text-base">(${r.rating_belanja || 0
        }/5)</span></p>
            <p class="text-xs text-gray-700 italic">${r.komentar || "Tidak ada komentar."
        }</p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
          </div>
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (!payload.nama_supplier || !payload.alamat || !payload.kontak) {
      showToast(
        "Mohon lengkapi semua field supplier yang wajib diisi!",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      const existing = await apiFetch(
        `/supplier?nama_supplier=${encodeURIComponent(payload.nama_supplier)}`
      );
      const found = existing.find(
        (s) =>
          s.nama_supplier.toLowerCase() === payload.nama_supplier.toLowerCase() &&
          s.kontak === payload.kontak
      );
      if (found) {
        showToast("Supplier dengan nama dan kontak ini sudah ada!", "warning");
        return;
      }
    
      // ✅ Submit jika tidak duplikat
...
async function loadSupplier() {
  ...
  const namaFilter = document
    .getElementById("filter-nama-supplier")
    .value.trim(); // ID unik
  const url = namaFilter
    ? `/supplier?nama_supplier=${encodeURIComponent(namaFilter)}`
    : "/supplier";
  ...
  try {
    ...
    list.innerHTML = "";
    if (data.length === 0) {
      list.innerHTML = `<div class="bg-gray-100 p-4 rounded-lg text-gray-600 text-center col-span-full">Tidak ada supplier ditemukan. ${namaFilter
        ? `Coba hapus filter "${namaFilter}".`
    ...
    data.forEach((s) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">Supplier: <span class="text-indigo-700">${s.nama_supplier || "N/A"
        }</span></h4>
            <p class="text-sm text-gray-600">ID: <span class="font-mono">${s.id_supplier || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Alamat: <span class="font-medium">${s.alamat || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Kontak: <span class="font-mono">${s.kontak || "N/A"
        }</span></p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
          </div>
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    const payload = {
      ...
      nomor_telepon: document
        .getElementById("nomor_telepon_karyawan")
        .value.trim(), // ID unik
    };

    if (!payload.nama_karyawan || !payload.jabatan || !payload.nomor_telepon) {
      showToast(
        "Mohon lengkapi semua field karyawan yang wajib diisi!",
        "warning"
      );
      return;
    }
    ...
    try {
      // 🔍 Duplikat check
      const existing = await apiFetch("/karyawan");
      const found = existing.find(
        (k) =>
          k.nama_karyawan.toLowerCase() === payload.nama_karyawan.toLowerCase() &&
          k.nomor_telepon === payload.nomor_telepon
      );
      if (found) {
        showToast("Karyawan dengan nama dan nomor ini sudah ada!", "warning");
        return;
      }
...
async function loadKaryawan() {
  ...
  try {
    ...
    data.forEach((k) => {
      ...
      card.innerHTML = `
          <div>
            <h4 class="text-lg font-semibold text-gray-900 mb-1">Nama: <span class="text-red-700">${k.nama_karyawan || "N/A"
        }</span></h4>
            <p class="text-sm text-gray-600">ID: <span class="font-mono">${k.id_karyawan || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Jabatan: <span class="font-medium">${k.jabatan || "N/A"
        }</span></p>
            <p class="text-sm text-gray-600">Telepon: <span class="font-mono">${k.nomor_telepon || "N/A"
        }</span></p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
          </div>
